...
It's an intuitive definition. It quantifies how much information we are *really* sending for each symbol we put on the channel. It remains constant if we concatenate multiple messages together using the same code. Note that here we are using the convention of measuring information in bits ($$\log x$$ is short for $$\log_2 x$$ ) but we could have used any other base. It's just a scaling constant. Lower rates indicate that we are using a lot of symbols to convey our message(s), while a higher rate means fewer symbols and more information is conveyed "per symbol". 
...
In other words, given a noiseless channel and a large $$k$$ there is a code that approximately[^fixed-n-rate] follows $$(M^k, k H_i )$$. Assuming a uniform message distribution, we have $$H_i = \log_i M$$. Its rate is:
...
as the probability of decoding the wrong message given that the original message was $$m$$.
...
In the next section, we introduce some concepts that will help us prove this statement.
...
where $$x^n$$ denotes the whole sequence. There are some properties of typical sets that we are going to use in short.

We now make an extension regarding pairs of random variables. Take a pair of (not necessarily independent) random variables $$(X,Y)$$. We denote a sequence of $$n$$ samples of this pair as $$ (x^n,y^n) $$. We define $$ (x^n,y^n) $$ to be in the jointly typical set $$J_\epsilon^n$$ if $$x^n$$ is $$\epsilon$$-typical, $$y^n$$ is $$\epsilon$$-typical and $$(x^n,y^n)$$ is $$\epsilon$$-typical.
...
The proof is very straightforward but it uses properties of typical sets.
...
Now we go to our main theorem. It says when a rate is achievable. I think it's best to go blind into the proof, so we won't state the theorem beforehand. What we are trying to do is find some code with some non-zero rate and a vanishing probability of error.
...
and so we have the pair of random variables $$(X^n_{enc},X^n_{dec})$$ denoting the input codeword and its result on the other side of the channel. Note that this process of choosing the codewords is only done *when constructing the codebook*. Once the samples are drawn for each codeword, $$f$$ is deterministic.

For the decoder side, we implement a simple policy. The decoder is going to see $$X^n_{dec}$$ and decide the message with the following criteria:
...
How likely are we to make a mistake *on average* across all possible codebooks constructed in this way?
...
C. And they are completely independent (as this codeword was chosen independently of what was in $$f(1)$$).
...
So, if we pick some $$m$$ then, on average, a random codebook (with a rate below $$I$$) will have a small probability of error. Then, one of those codes must also have a small probability of error. It looks great but there's a subtlety. We proved it for *a particular* $$m$$ [^changing-m]. It may very well be that some codes are only good for $$m_1$$ and some others for $$m_2$$. We want to know that a codebook is good for all messages. If we check the average *arithmetic mean of* $$\lambda_m$$ we get we can still use our previous bound, that is:
...
But it approaches $$R$$ so it's good enough. We can (asymptotically) transmit at a rate of $$R$$ bits per symbol while simultaneously guaranteeing an arbitrarily small chance of error. So, $$R < I({X}_{enc}, {X}_{dec})$$ is achievable!
...
<!-- 
### Some formal definitions. Distortion and Rate

Our original object come from set $$\mathcal{X}$$ while the reconstructed object come from set $$\hat{\mathcal{X}}$$ (usually $$\mathcal{X} = \hat{\mathcal{X}}$$). We assume a distortion function $$d: \mathcal{X} \times \hat{\mathcal{X}}  \rightarrow \mathbb{R}+$$ quantifying how bad a guess is. It's a bit anticlimactic that we need to provide the distortion function ourselves, but it makes sense. Different applications have different interests (I talk a bit about it in the introduction [here](../../../../../2021/12/20/lossy-compression/)). We extend the distortion function for multiple chains of objects by 

$$ d(x^n, \hat{x}^n) = \frac{1}{n} \sum d(x_i, \hat{x}_i) $$


We are going to make our encoder map inputs into discrete possibilities. Codes are going to take $$n$$ inputs and map it to one of such values. A $$(2^{nR}, n)$$ code is a code that maps $$n$$ inputs into one of $$\lceil  2^{nR} \rceil$$ values. Such code is said to have rate $$R$$. You are sending R bits per input.


Given a code and a distribution from $$\mathcal{X} $$ values, we define the distortion as

$$ D = \mathbb{E}[d(X^n, \hat{X^n})] $$

A rate-distortion pair $$(R,D)$$ is said to be achievable if there's a sequence of codes $$C_n$$ such that $$ n \rightarrow \infty $$ its rate goes to $$R$$ and $$ \lim_{n \rightarrow \infty} D(C_n)  \leq D $$. 

We define $$R(D)$$ as the infimum $$R$$ such that $$(R,D)$$ is achievable. Similarly, $$D(R)$$ is the infimum $$D$$ such that $$(R,D)$$ is achievable. -->

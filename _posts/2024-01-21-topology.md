...
Generally speaking, when talking about a subset of $$R^k$$ we'll assume we are using the induced topology from euclidean space unless we specifically state otherwise.
...
Note that we are using these new words to refer to known concepts, two topologies are equal if they are coarser and finer at the same time.
...
Now that we defined topologies we can talk more freely about the concept of continuous functions in a general sense without limiting ourselves to euclidean space. It's pretty cool. 
